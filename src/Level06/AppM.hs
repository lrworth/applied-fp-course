...
instance Functor (AppM e) where
  ...
  fmap f = AppM . ((pure . fmap f) <=< runAppM)
...
instance Monad (AppM e) where
  ...
  m >>= f = AppM (either (pure . Left) (runAppM . f) =<< runAppM m)
...
instance MonadError e (AppM e) where
  ...
  catchError go handler = AppM (either (runAppM . handler) (pure . Right) =<< runAppM go)
...
instance Bifunctor AppM where
  ...
  bimap f g = AppM . (pure . bimap f g <=< runAppM)
...
hoistToAppM :: IO (Either e a) -> AppM e a
hoistToAppM = AppM
